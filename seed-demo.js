...
async function main() {
  ...
  try {
    ...
    for (const inv of INVOICES) {
      const invDate = dateStr(offsetDate(today, inv.di));
      const dueDate = dateStr(offsetDate(today, inv.dd));
      const paidAt  = inv.da != null ? chicagoTs(dateStr(offsetDate(today, inv.da)), 12) : null;
      await client.query(
