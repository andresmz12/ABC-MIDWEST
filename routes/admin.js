...
router.post('/scheduled-jobs', async (req, res) => {
  try {
    const { title, scheduled_date, assigned_to, location, notes, reminder_date } = req.body;
    ...
    const { rows } = await query(
      'INSERT INTO scheduled_jobs (title, scheduled_date, assigned_to, location, notes, reminder_date) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',
      [title, scheduled_date, assignedArr, location || null, notes || null, reminder_date || null]
    );
...
router.put('/scheduled-jobs/:id', async (req, res) => {
  try {
    ...
    const assignedArr = Array.isArray(assigned_to) ? assigned_to : [];
    await query(
      'UPDATE scheduled_jobs SET title=$1, scheduled_date=$2, assigned_to=$3, location=$4, notes=$5, reminder_date=$6 WHERE id=$7',
      [title, scheduled_date, assignedArr, location || null, notes || null, reminder_date || null, req.params.id]
    );
    res.json({ success: true });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Server error' }); }
});
